...
with pd.read_csv(input_path, sep='\t', low_memory=False, encoding='utf-8', on_bad_lines="skip", chunksize=10000) as reader:
    for raw_data in reader:
        ...
        # Step 3: standardize and normalize the raw dataset2
        for entity in entity_mapping:
            for attribute in entity_mapping[entity]:
                ...
                elif attribute["type"] == "INT" or attribute["type"] == "DECIMAL":
                    tables[entity][attribute["name"]] = tables[entity][attribute["name"]].replace("", None) # Might need to get replaced depending on how the database works
